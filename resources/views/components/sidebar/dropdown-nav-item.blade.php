<li class="relative px-2 py-1" x-data="{ {{$active}}:  @if(\Request::is($uri)) true @else false @endif }">
    <div class="inline-flex items-center justify-between w-full text-base font-semibold transition-colors duration-150 text-gray-500   cursor-pointer"
        x-on:click="{{$active}} = !{{$active}}">
        <span class="inline-flex items-center  text-sm font-semibold text-white hover:text-red-500 ">
            {{ $icon }}
    ...
    <div x-show.transition="{{$active}}" x-cloak>
        <ul x-transition:enter="transition-all ease-in-out duration-300" x-transition:enter-start="opacity-25 max-h-0"
            ...
            class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium  rounded-md shadow-inner  bg-red-500"
            aria-label="submenu">
